function compare_controllers(config)
% Compare DMC, MPC, DeePC, and SPC controller performance
% IMPROVED: Uses centralized configuration for colors and parameters
    ...
    load('results/SPC_results.mat', 'SPC_results'); % <-- ADDED

    % Get colors from config (with SPC fallback)
    colors = config.plotting.colors;
    if ~isfield(colors, 'SPC')
        colors.SPC = [0.49, 0.18, 0.56]; % purple-ish
    end

    controller_order = {'MPC', 'DMC', 'DeePC', 'SPC'};
    controller_labels = {'MPC', 'DMC', 'DeePC', 'SPC'};

    %% Print comparison table
    fprintf('\n========== Performance Comparison ==========\n');
    fprintf('%-20s %-15s %-15s %-15s %-15s\n', 'Metric', 'MPC', 'DMC', 'DeePC', 'SPC');
    fprintf('%-20s %-15s %-15s %-15s %-15s\n', repmat('-', 1, 20), repmat('-', 1, 15), ...
            repmat('-', 1, 15), repmat('-', 1, 15), repmat('-', 1, 15));
    fprintf('%-20s %-15.3f %-15.3f %-15.3f %-15.3f\n', 'MAE [°C]', ...
            MPC_results.MAE, DMC_results.MAE, DeePC_results.MAE, SPC_results.MAE);
    fprintf('%-20s %-15.3f %-15.3f %-15.3f %-15.3f\n', 'RMSE [°C]', ...
            MPC_results.RMSE, DMC_results.RMSE, DeePC_results.RMSE, SPC_results.RMSE);
    fprintf('%-20s %-15.3f %-15.3f %-15.3f %-15.3f\n', 'ISE', ...
            MPC_results.ISE, DMC_results.ISE, DeePC_results.ISE, SPC_results.ISE);
    fprintf('%-20s %-15.3f %-15.3f %-15.3f %-15.3f\n', 'IAE', ...
            MPC_results.IAE, DMC_results.IAE, DeePC_results.IAE, SPC_results.IAE);
    fprintf('%-20s %-15.3f %-15.3f %-15.3f %-15.3f\n', 'Control Effort', ...
            MPC_results.control_effort, DMC_results.control_effort, DeePC_results.control_effort, SPC_results.control_effort);
    fprintf('============================================\n\n');

    % Find best performer for each metric
    [~, best_mae]  = min([MPC_results.MAE,  DMC_results.MAE,  DeePC_results.MAE,  SPC_results.MAE]);
    [~, best_rmse] = min([MPC_results.RMSE, DMC_results.RMSE, DeePC_results.RMSE, SPC_results.RMSE]);
    [~, best_ise]  = min([MPC_results.ISE,  DMC_results.ISE,  DeePC_results.ISE,  SPC_results.ISE]);

    fprintf('Best MAE:  %s (%.3f °C)\n', controller_order{best_mae}, ...
            min([MPC_results.MAE, DMC_results.MAE, DeePC_results.MAE, SPC_results.MAE]));
    fprintf('Best RMSE: %s (%.3f °C)\n', controller_order{best_rmse}, ...
            min([MPC_results.RMSE, DMC_results.RMSE, DeePC_results.RMSE, SPC_results.RMSE]));
    fprintf('Best ISE:  %s (%.3f)\n', controller_order{best_ise}, ...
            min([MPC_results.ISE, DMC_results.ISE, DeePC_results.ISE, SPC_results.ISE]));
    ...
    plot(SPC_results.t, SPC_results.y, 'Color', colors.SPC, 'LineWidth', 1.5);
    xlabel('Time [s]'); ylabel('Temperature [°C]');
    legend('Setpoint', 'MPC', 'DMC', 'DeePC', 'SPC', 'Location', 'best');
    ...
    stairs(SPC_results.t, SPC_results.u, 'Color', colors.SPC, 'LineWidth', 1.5);
    xlabel('Time [s]'); ylabel('Heater Power [%]');
    legend('MPC', 'DMC', 'DeePC', 'SPC', 'Location', 'best');
    ...
    plot(SPC_results.t, abs(SPC_results.error), 'Color', colors.SPC, 'LineWidth', 1.5);
    ...
    plot(SPC_results.t(idx), SPC_results.y(idx), 'Color', colors.SPC, 'LineWidth', 1.5);
    xlabel('Time [s]'); ylabel('Temperature [°C]');
    ...
    plot(SPC_results.t, abs(du_spc), 'Color', colors.SPC, 'LineWidth', 1.5);
    ...
    integrated_data = [MPC_results.ISE, MPC_results.IAE; ...
                       ...
                       SPC_results.ISE, SPC_results.IAE];
    ...
    control_efforts = [MPC_results.control_effort; ...
                       ...
                       SPC_results.control_effort];
    b = bar(control_efforts);
    b.FaceColor = 'flat';
    b.CData = [colors.MPC; colors.DMC; colors.DeePC; colors.SPC];
    set(gca, 'XTickLabel', controller_labels);
    ylabel('Total Control Effort');
    title('Control Effort Comparison');
    grid on;

    sgtitle('Controller Performance Comparison: MPC vs DMC vs DeePC vs SPC');
    ...
    histogram(SPC_results.error, 30, 'Normalization', 'probability', ...
              'FaceColor', colors.SPC, 'FaceAlpha', 0.5, 'DisplayName', 'SPC');
    ...
    plot(SPC_results.t, cumsum(abs(SPC_results.error))*SPC_results.t(2), ...
         'Color', colors.SPC, 'LineWidth', 1.5);
    xlabel('Time [s]');
    ...
    settling_spc   = compute_settling_metrics(SPC_results.t, SPC_results.error, threshold);

    settling_values = [settling_mpc.mean; settling_dmc.mean; settling_deepc.mean; settling_spc.mean];
    ...
    comparison_summary.SPC = SPC_results;
    ...
    for i = 1:length(changes)
        ...
        while idx_end < length(error)
            if all(abs(error(idx_end:min(idx_end+10, length(error)))) < threshold)
                settling_times = [settling_times, (idx_end - idx_start) * (t(2) - t(1))]; %#ok<AGROW>
                break;
            end
            idx_end = idx_end + 1;
        end
