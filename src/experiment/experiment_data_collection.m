...
% UPDATE: Adds a second multisine dataset with different T0 and RNG seed
    ...
    % FIX: ensure results/data exists (saves below need it)
    if ~exist(fullfile('results','data'), 'dir')
        mkdir(fullfile('results','data'));
    end
    ...
    %% ========== EXPERIMENT 3: MULTISINE RESPONSE (1) ==========
    fprintf('Running Multisine Response Experiment (1)...\n');
    ...
    %% ========== EXPERIMENT 3B: MULTISINE RESPONSE (2) ==========
    fprintf('Running Multisine Response Experiment (2) [different T0 + seed]...\n');

    % Defaults if you haven't added these to config_simulation() yet
    if isfield(config, 'data_collection') && isfield(config.data_collection, 'multisine2_seed')
        seed2 = config.data_collection.multisine2_seed;
    else
        seed2 = 166; % fallback
    end

    if isfield(config, 'data_collection') && isfield(config.data_collection, 'multisine2_T0_delta')
        T0_delta = config.data_collection.multisine2_T0_delta;
    else
        T0_delta = 5.0; % fallback: +5°C
    end

    params2 = params;
    params2.T0 = params.T0 + T0_delta;

    rng(seed2);
    phases2 = 2*pi*rand(1, n_freq);

    Q_multisine2 = @(t) multisine_offset + multisine_amplitude * ...
        sum(sin(2*pi*freqs'.*t + phases2'), 1) / n_freq;

    [T_multisine2_clean, t_multisine2, Q_multisine2_data] = thermal_model(Q_multisine2, params2);

    [T_multisine2, noise_stats_multisine2] = add_measurement_noise(T_multisine2_clean, config);

    fprintf('  RNG seed: %d\n', seed2);
    fprintf('  T0 used: %.2f °C (delta: %+0.2f °C)\n', params2.T0, T0_delta);

    if config.noise.enable
        fprintf('  Noise Statistics:\n');
        fprintf('    Achieved SNR: %.2f dB\n', noise_stats_multisine2.SNR_achieved_dB);
        fprintf('    Noise RMSE: %.4f °C\n', noise_stats_multisine2.RMSE);
    end

    multisine2_data = struct('t', t_multisine2, 'T', T_multisine2, 'T_clean', T_multisine2_clean, ...
                             'Q', Q_multisine2_data, ...
                             'params', params2, 'freqs', freqs, 'phases', phases2, ...
                             'amplitude', multisine_amplitude, 'offset', multisine_offset, ...
                             'seed', seed2, ...
                             'T0_delta', T0_delta, ...
                             'noise_stats', noise_stats_multisine2, ...
                             'config', config);
    save('results/data/multisine_response_data_2.mat', 'multisine2_data');
    fprintf('  Saved: results/data/multisine_response_data_2.mat\n\n');
    ...
    title(sprintf('Step - Noise (\\sigma=%.3f)', noise_stats_step.noise_std));
    grid on;
    ...
    xlabel('Time [s]'); ylabel('Noise [°C]');
    title(sprintf('Impulse - Noise (\\sigma=%.3f)', noise_stats_impulse.noise_std));
    grid on;

    % Multisine response (1)
    subplot(5,3,7);
    plot(t_multisine, Q_multisine_data, 'b', 'LineWidth', config.plotting.linewidth);
    xlabel('Time [s]'); ylabel('Heater Input [%]');
    title(sprintf('Multisine (1) - Input (seed=%d)', seed1));
    grid on;

    subplot(5,3,8);
    plot(t_multisine, T_multisine_clean, 'k--', 'LineWidth', 1, 'DisplayName', 'Clean'); hold on;
    plot(t_multisine, T_multisine, 'r', 'LineWidth', config.plotting.linewidth, 'DisplayName', 'Noisy');
    xlabel('Time [s]'); ylabel('Temperature [°C]');
    title(sprintf('Multisine (1) - Output (SNR: %.1f dB)', noise_stats_multisine.SNR_achieved_dB));
    legend('Location', 'best');
    grid on;
    ...
    xlabel('Time [s]'); ylabel('Noise [°C]');
    title(sprintf('Multisine (1) - Noise (\\sigma=%.3f)', noise_stats_multisine.noise_std));
    grid on;

    % Multisine response (2)
    subplot(5,3,10);
    plot(t_multisine2, Q_multisine2_data, 'b', 'LineWidth', config.plotting.linewidth);
    xlabel('Time [s]'); ylabel('Heater Input [%]');
    title(sprintf('Multisine (2) - Input (seed=%d)', seed2));
    grid on;

    subplot(5,3,11);
    plot(t_multisine2, T_multisine2_clean, 'k--', 'LineWidth', 1, 'DisplayName', 'Clean'); hold on;
    plot(t_multisine2, T_multisine2, 'r', 'LineWidth', config.plotting.linewidth, 'DisplayName', 'Noisy');
    xlabel('Time [s]'); ylabel('Temperature [°C]');
    title(sprintf('Multisine (2) - Output (SNR: %.1f dB)', noise_stats_multisine2.SNR_achieved_dB));
    legend('Location', 'best');
    grid on;

    subplot(5,3,12);
    plot(t_multisine2, T_multisine2 - T_multisine2_clean, 'Color', [0.5 0.5 0.5], 'LineWidth', 1);
    xlabel('Time [s]'); ylabel('Noise [°C]');
    title(sprintf('Multisine (2) - Noise (\\sigma=%.3f)', noise_stats_multisine2.noise_std));
    grid on;
    ...
    subplot(5,3,14);
    plot(t_doublet, T_doublet_clean, 'k--', 'LineWidth', 1, 'DisplayName', 'Clean'); hold on;
    plot(t_doublet, T_doublet, 'r', 'LineWidth', config.plotting.linewidth, 'DisplayName', 'Noisy');
    xlabel('Time [s]'); ylabel('Temperature [°C]');
    ...
    xlabel('Time [s]'); ylabel('Noise [°C]');
    title(sprintf('Doublet - Noise (\\sigma=%.3f)', noise_stats_doublet.noise_std));
    grid on;
